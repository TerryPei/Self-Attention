...
class Dina(object):
    ...
    def get_log_Likelihood_matrix(self, P_success):
        ...
        X = self._X #注意底下不要直接使用self._X，否则会改变私有变量
...
class EmDina(Dina):
    ...
    def get_skill_distribution(self, posterior_matrix_normalize):
        #每种技能组合的人数分布, 对归一化后的矩阵行向量求和,
        return np.sum(posterior_matrix_normalize, axis=1)#1.此向量中每一个值代表的都是拥有该种技能组合的人数 2.若对此返回的向量求和，则返回的数值便是学生总数


    def get_init_eta_item_distribution(self, posterior_matrix_normalize):
        #简化代码：每道题都有做对做错都人数分布，eta0用于做错的人数分布， eta1用于做对的人数分布, L*J;
...
class McmcDina(Dina):
    ...
    def get_Q_slip_and_guess(self, skills, slip, guess, next_slip, next_guess):#guess. slip的转移矩阵
        ...
        Q_item_trans[Q_item_trans > 1] = 1 #j*1,每个值都是条件概率
        return Q_item_trans

    def update_slip_and_guess(self, skills, slip, guess):#根据马尔可夫链更新一次slip和guess
    ...
    def get_Q_skills(self, slip, guess, skills, next_skills):
        ...
        Q_skills_trans[Q_skills_trans>1] = 1 #L*1， 每个值都是条件概率
